...
class Stb_model extends CI_Model
{
   ...
   var $maTv = array();
    ...
    public function retrievChainesList()
    {
        ...
        $sql = "	
                select C.dept_ids,C.bouq_uid,D.designation as nom_bouquet, C.cat_uid, C.nom_categorie, C.chain_uid, C.nom_chaines, C.logo as img_logo, C.icon as img_icon FROM 
                  (
                     select E.dept_ids, E.bouq_uid, E.cat_uid, F.nom as nom_categorie, E.chain_uid, G.designation as nom_chaines, G.logo,G.icon FROM `m_m_bouq_chain` E, `mod_wa_categories` F, `mod_wa_chaines` G WHERE F.uid= E.cat_uid AND G.uid=E.chain_uid AND E.disable = 0
                  ) C
                 INNER JOIN `mod_wa_bouquets` D
                 ON C.bouq_uid = D.uid WHERE D.disable = 0 AND C.dept_ids like '%?%' ORDER BY D.designation,C.nom_categorie  
                ";
